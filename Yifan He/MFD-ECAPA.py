...
'''
import torch
# print("PyTorch CUDA 编译版本：", torch.version.cuda)
...
class iAFF_Stepwise(nn.Module):
    '''
    分步多特征融合 iAFF
    '''

    def __init__(self, channels=512, r=4):
        super(iAFF_Stepwise, self).__init__()
    ...
    def forward(self, x1, x2, x3):
        ...
        x_ab = x1 * wei1 + x2 * (1 - wei1)
        
        # 第二阶段：融合 AB 和 C
        xb = x_ab + x3
        xl2 = self.local_att2(xb)
        xg2 = self.global_att2(xb)
        xlg2 = xl2 + xg2
        wei2 = self.sigmoid(xlg2)        
        x_abc = x_ab * wei2 + x3 * (1 - wei2)     
        return x_abc    
...
class FbankAug(nn.Module):
    ...
    def __init__(self, freq_mask_width = (0, 8), time_mask_width = (0, 10)): # 注意倍数
...
class MFD_ECAPA(nn.Module): # 加深模型

    def __init__(self, C):
        super(MFD_ECAPA, self).__init__()
        ...
        # 这段是关于计算特征
        self.specaug = FbankAug()  # Spec augmentation 这是数据增强技术
        self.conv1 = nn.Conv1d(80, C, kernel_size=5, stride=1, padding=2)
        self.relu = nn.ReLU()
        self.bn1 = nn.BatchNorm1d(C)
        # 定义独立的瓶颈层
        self.layer1_1 = Bottle2neck(C, C, kernel_size=3, dilation=2, scale=8)
        ...
        self.layer2_1 = Bottle2neck(C, C, kernel_size=3, dilation=3, scale=8)
        ...
        self.layer3_1 = Bottle2neck(C, C, kernel_size=3, dilation=4, scale=8)
        ...
        self.step = iAFF_Stepwise()

    def forward(self, x, aug, current_epoch):
        with torch.no_grad():
        ...
        x13 = self.layer1_3(x12)
        x1 = self.step(x11, x12, x13)

        # 第二组瓶颈层和融合
        x21 = self.layer2_1(x + x1)
        x22 = self.layer2_2(x21)
        x23 = self.layer2_3(x22)
        x2 = self.step(x21, x22, x23)

        # 第三组瓶颈层和融合
        x31 = self.layer3_1(x + x2)
        x32 = self.layer3_2(x31)
        x33 = self.layer3_3(x32)
        x3 = self.step(x31, x32, x33)
        ...
        sg = torch.sqrt((torch.sum((x ** 2) * w, dim=2) - mu ** 2).clamp(min=1e-9))
